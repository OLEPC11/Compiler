...
String s4= " ";
    ...
    public String CodeGen() {
        ...
        if (functionDeclaration != null) {
            s1= functionDeclaration.CodeGen();
        }
        if(importStatement!=null)
        {
            s5= importStatement.CodeGen();
        }
        if (returnStatement != null) {
            s3= returnStatement.CodeGen();
        }

        if (expressionStatement != null) {
            s4= expressionStatement.CodeGen();
        }

        if (variableStatement != null) {
            s= variableStatement.CodeGen();
        }
        if(exportStatement!=null)
        {
            exportStatement.CodeGen();
        }
        return s+s1+s3+s4+s5;
    }
}
